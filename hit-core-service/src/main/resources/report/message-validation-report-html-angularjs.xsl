<xsl:stylesheet exclude-result-prefixes="map" version="2.0"
	...
	<xsl:template match="report:HL7V2MessageValidationReport">
		<!-- <div ng-app="validationApp" ng-controller="ValidationController as vm"> -->
	...
	<xsl:template match="report:HeaderReport">
		<xsl:if test="$withHeader = boolean('true')">
			<div class="report-section">
				<table class="forumline title-background" width="100%" cellspacing="1" cellpadding="10">
	...
	<xsl:template match="report:SpecificReport">
		...
		<xsl:apply-templates select="report:MetaData/report:Context/context:FailureInterpretation" />
		<xsl:call-template name="Summary" />
		<xsl:call-template name="Assertions">
			<xsl:with-param name="classification" select="'error'" />
			<xsl:with-param name="count" select="../report:HeaderReport/message:ErrorCount" />
			<xsl:with-param name="color" select="'red'" />
			<xsl:with-param name="msg" select="'Errors'" />
		</xsl:call-template>
		<xsl:call-template name="Assertions">
			<xsl:with-param name="classification" select="'alert'" />
			<xsl:with-param name="count" select="../report:HeaderReport/message:AlertCount" />
		...
		<xsl:call-template name="Assertions">
			...
			<xsl:with-param name="count" select="../report:HeaderReport/message:WarningCount" />
		...
		<div class="report-section">
			<table class="forumline" width="100%" cellspacing="1" cellpadding="2">
				<tbody>
					<tr class="row1">
						<td>
							Validation <xsl:value-of select="$msg" />
						</td>
						<td align="right">
							...
							<input type="checkbox"
								ng-model="vm.categoryVisible['{$classification}']"
								ng-init="vm.categoryVisible['{$classification}']={$classification='error'}"
							/>
						</td>
					</tr>
				</tbody>
			</table>
			<table class="forumline cf-report-category" width="100%" cellspacing="1" cellpadding="2"
				ng-show="vm.categoryVisible['{$classification}']">
		...
		<tbody>
			<tr>
				...
				<td class="row5 border_bottom" align="right" style="width:70%">
					Count : <xsl:value-of select="count(current-group())" />
					<input type="checkbox"
						ng-model="vm.groupVisible['{$id}']"
						ng-init="vm.groupVisible['{$id}']=true"
					/>
				</td>
			</tr>
		</tbody>
		<xsl:for-each select="current-group()">
			<tbody ng-show="vm.groupVisible['{$id}']"
				class="border_bottom alternate{position() mod 2} border_bottom {$id}">
				<tr class="border_bottom">
					<td class="row3 border_right border_bottom" rowspan="5" style="width:20px;">
				...
				<tr class="row3 border_bottom">
					<td rowspan="3" class="border_right border_bottom" style="width:100px;">Location:</td>
					<xsl:if test="report:Location/report:Line">
						<td colspan="2" style="font-weight: bold">
							Line: <xsl:value-of select="report:Location/report:Line" />
						</td>
					</xsl:if>
				</tr>
				<xsl:if test="report:Location/report:Column">
					<tr class="row3 border_bottom">
						<td colspan="2" style="font-weight: bold">
							Column: <xsl:value-of select="report:Location/report:Column" />
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="report:Location/report:Path">
					<tr class="row3 border_bottom">
						<td colspan="2" style="font-weight: bold;" class="border_bottom">
							Path: <xsl:value-of select="report:Location/report:Path" />
						</td>
					</tr>
				</xsl:if>
			</tbody>
	...
	<xsl:template match="report:MetaData/report:Message">
		<xsl:variable name="msgId" select="generate-id()" />
		<div class="report-section">
			<table class="forumline" width="100%" cellspacing="1" cellpadding="2" id="msgC{$msgId}">
				<tbody>
					<tr class="row1">
						<th style="border-bottom:2pt #005C99 solid" align="left">
							Message
							<xsl:choose>
								<xsl:when test="report:Er7MessageHexFormatted">
									<button type="button" ng-click="vm.separatorVisible['{$msgId}']=true">
										Show Separators
									</button>
								</xsl:when>
							</xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="report:Er7MessageHexFormatted">
					<table class="forumline" width="100%" cellspacing="1" cellpadding="2"
						id="msgS{$msgId}" ng-show="vm.separatorVisible['{$msgId}']">
						<tbody>
							<tr class="row1">
								<th style="border-bottom:2pt #005C99 solid" align="left">
									Message
									<button type="button" ng-click="vm.separatorVisible['{$msgId}']=false">
										Hide Separators
									</button>
								</th>
							</tr>
				...
				<tbody>
					...
					<tr class="border_bottom">
						<td class="row6 " style="color: red; font-weight: bold">
							<input type="checkbox"
								ng-model="vm.categoryVisible['error']"
								ng-init="vm.categoryVisible['error']=true"
							/>
							<xsl:value-of select="../report:HeaderReport/message:ErrorCount" /> Errors
						</td>
					</tr>
					<tr class="border_bottom">
						<td class="row6 " style="color: maroon; font-weight: bold">
							<input type="checkbox"
								ng-model="vm.categoryVisible['alert']"
								ng-init="vm.categoryVisible['alert']=false"
							/>
							<xsl:value-of select="../report:HeaderReport/message:AlertCount" /> Alerts
						</td>
					</tr>
					<tr class="border_bottom">
						<td class="row6 " style="color: gold; font-weight: bold">
							<input type="checkbox"
								ng-model="vm.categoryVisible['warning']"
								ng-init="vm.categoryVisible['warning']=false"
							/>
							<xsl:value-of select="../report:HeaderReport/message:WarningCount" /> Warnings
						</td>
					</tr>
				</tbody>
			</table>
	...
	<xsl:template match="report:MetaData/report:Context/context:FailureInterpretation">
		<xsl:variable name="mfiId" select="concat('mfi', generate-id())"/>
		<div class="report-section">
			<table class="forumline" width="100%" cellspacing="1" cellpadding="2">
				<tr class="row1">
					<th style="border-bottom:2pt #005C99 solid" align="left">Failures interpretation</th>
					<td align="right" style="border-bottom:2pt #005C99 solid">
						<button type="button" ng-click="vm.failureVisible['{$mfiId}']=!vm.failureVisible['{$mfiId}']">
							<span ng-show="!vm.failureVisible['{$mfiId}']">View</span>
							<span ng-show="vm.failureVisible['{$mfiId}']">Hide</span>
						</button>
					</td>
				</tr>
				<tbody ng-show="vm.failureVisible['{$mfiId}']" id="{$mfiId}">
					<tr>
					...
					<xsl:for-each select="context:MessageFailure">
						<tr class="border_bottom alternate{position() mod 2}">
